...
QBcore.Functions.CreateCallback('qb-expandableVehicleTrunk:server:getDataFromNearbyVehicle',
    function(source, cb, parameters)
        local src = source
        local vehicleInfoFromDatabase
        if parameters["isVehicleStopped"] == 1 and parameters["isEngineRunning"] == false then
            vehicleInfoFromDatabase = MySQL.Sync.fetchAll(
