...
local function makeWeightUpgradesString(source, weightUpgrades, request, vehicleData)
    local tmp = {}
    ...
    -- we didn't found model use Default values
    for key, value in pairs(request.Upgrades) do
        -- just read from server config file rather than what we got from client! even tho thay samething
...
local function saveCarWeight(source, weightUpgrades, request, vehicleData)
    -- save car Weight
...
function upgradeProcess(source, vehicleData, request)
...
local function upgradeWithDefaultValue(Data, vehicleInfoFromDatabase)
    return upgradesInfromation(Data, DefaultMetaData.Default, vehicleInfoFromDatabase)
end
